...
(function($, window, document, undefined) {
	...
	Picker.prototype.setItems = function(items) {
		...
		self.items = items || [];
		var buffer = [];
		var textKey = (self.options.textKey==undefined?"text":self.options.textKey);
		self.items.forEach(function(item) {
			if (item !== null && item !== undefined) {
				buffer.push('<li>' + (item[textKey] || item) + '</li>');
			}
		});
	...
	Picker.prototype.getSelectedValue = function() {
		var self = this;
		var valueKey = (self.options.valueKey==undefined?"value":self.options.valueKey);
		return (self.items[self.getSelectedIndex()] || {})[valueKey];
	};

	Picker.prototype.getSelectedText = function() {
		var self = this;
		var textKey = (self.options.textKey==undefined?"value":self.options.textKey);
		return (self.items[self.getSelectedIndex()] || {})[textKey];
	};
